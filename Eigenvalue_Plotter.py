...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr  6 18:08:10 2017
...
# Get a list of all of the eigenvalues across the simulation
eigenvalues = []
stiffratios = []
...
for i in range(len(tlist)):
    ...
    eigsnozero = [abs(i) for i in eigs if i != 0]
    stiffratios.append(max(eigsnozero) / min(eigsnozero))
eigenvalues = np.array(eigenvalues)
stiffratios = np.array(stiffratios)
...
mineig = 0.
for i in range(13):
    if max(eigenvalues[:,i]) > maxeig:
        maxeig = max(eigenvalues[:,i])
    if min(eigenvalues[:,i]) < mineig:
        mineig = min(eigenvalues[:,i])
    pyl.xlabel('Time (s)', fontsize=fs)
    pyl.ylabel('Eigenvalue')
    pyl.scatter(tlist, eigenvalues[:,i], s=0.1)
pyl.xlim(tstart,tstop)
pyl.ylim(0.01,maxeig)
pyl.yscale('log')
print('Maximum Eigenvalue: {}'.format(maxeig))
print('Minimum Eigenvalue: {}'.format(mineig))
pyl.show()
"""
pyl.figure(0,figsize=(6, 4.5), dpi=400)
pyl.xlabel('Time (s)')
pyl.ylabel('Stiffness Ratio')
pyl.plot(tlist, stiffratios)
pyl.yscale('log')
pyl.savefig('Stiffness_Ratio.png')
