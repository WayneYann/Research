...
def intDriver(tim, dt, y_global, mu, setup, CSPtols, *RHSparam):
    ...
    solver.set_initial_value(y0_local, t0)
    if noRHSparam:
        pass
    else:
...
def dobasicallyeverything(i,numbin,problem,useN2,jacfun,dt,mu,setup,CSPtols,firstderiv):
    tim = 0.0
...
if ((problem != 'H2' and problem != 'GRIMech') or autoignition):
    ...
    results = [pool.apply_async(dobasicallyeverything, args=(i,numbin,problem,useN2,jacfun,dt,mu,setup,CSPtols,firstderiv)) for i in range(int(len(pasr)/numbin))]
    print('HERE')
    t_end = time.time()
