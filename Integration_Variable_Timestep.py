...
if savedata:
    solfilename = equation + '_Solution_Explicit_Var_dt'
    inttimingfilename = equation + '_Int_Times_Explicit_Var_dt_' +\
        timer.strftime("%m_%d")
    workfilename = equation + '_FunctionWork_Explicit_Var_dt'
    simtimesfilename = equation + '_Sim_Times_Explicit_Var_dt'
    stepsizesfilename = equation + '_Stepsizes_Explicit_Var_dt'
    if PaSR:
        inttimingfilename = 'PaSR_' + inttimingfilename
        workfilename = 'PaSR_' + workfilename
        stepsizesfilename = 'PaSR_' + stepsizesfilename
    else:
        np.save(data_folder + solfilename, solution)
        np.save(data_folder + simtimesfilename, simtimes)
    np.save(data_folder + inttimingfilename, solutiontimes)
    np.save(data_folder + workfilename, functionwork)
    np.save(data_folder + stepsizesfilename, stepsizes)
